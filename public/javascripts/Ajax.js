...
$(document).ready(function() {
    $('#btnCreateAsset').click(function() {
        ...
        $.ajax(balanceUrl).then(function(res) {
            ...
            if(loopvar > 9){
                alert("Die Maximale Anzahl an Vorträgen ist erreicht."+"\n"+"Für neue Vorträge alte löschen!")
            }
            else{
    ...
    $('#btnDeleteAsset').click(function(req) {
        ...
        if (!assetName) {
            ...
            $.ajax(balanceUrl).then(function(res) {
                ...
                if(loopvar == 0){
                    alert("Keine Vorträge vorhanden")
                }
                else{
                ...
                $.ajax(transferUrl, {url: '/db_information'}).then(function(res) {
                    ...
                    if(!res){
                        ...
                        $.ajax(transferUrl).then(function(res) {  
                            ...
                            console.log(error)
                            if(success == false){ 
                                alert("Vortrag existiert nicht, bitte Eingabe überprüfen"+error)
                            }
                            else{
                                let success = res.success;
